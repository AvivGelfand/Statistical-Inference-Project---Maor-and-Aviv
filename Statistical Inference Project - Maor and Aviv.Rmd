...
Y_sig_squre_hat_mom <- y_squre - Ey_squre
Y_sig_squre_hat_mom
```
...
W
gamma.shape(W)
a_hat_MLE <- gamma.dispersion(W)
a_hat_MLE

# Moment method 
a_hat_moment <- (mean(BMI)^2)/((mean(BMI^2))-(mean(BMI)^2))
a_hat_moment
# mean(BMI)^2
lambda_hat_moment <- (mean(BMI))/((mean(BMI^2))-(mean(BMI)^2))
lambda_hat_moment
quants <- c(10,50,75,90)
quants_X <- qgamma(quants, shape = a_hat_moment, scale = lambda_hat_moment)
qgamma(10, shape = a_hat_moment, rate = lambda_hat_moment)
...
# alp <- seq(4,5,0.01)
# ell.alp <- dif.gam(alp)
# plot(alp,ell.alp,type='l')
# abline(h=0)
# 
# uniroot(dif.gam,c(4,5))
# 
# alpha.MLE <- uniroot(dif.gam,c(4,5))$root
# lambda.MLE <- alpha.MLE/mean.op
# 
# hist(optime,breaks=15,freq=FALSE)
# lines(x,dgamma(x,alpha.moment,lambda.moment),col='blue')
# lines(x,dlnorm(x,mu,sigma),col='red')
# x <- seq(0,10,0.1)
# lines(x,dgamma(x,alpha.MLE,lambda.MLE),col='black')
