...
#### 1) Recall of our previous project
...
```
'Both of the astimators are unbiased. we can see that mom astimator has smaller varity and hence smaller mse rate. therfore we will proceed the assignment with mom astimator'
...
quants_X
quantile(BMI, probs = quants) #for x
quantile(Life.expectancy, probs=quants) # for y
#1.d
...
p_value <- (mean(y_lower) - mean(y_high))/(sd)


S2_n = n-1(var(y_lower))
